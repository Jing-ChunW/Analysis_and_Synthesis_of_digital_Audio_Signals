...
% FILENAME_source = 'the_dog_is_on_top_of_the_shed';
fname = 'draw_16';
[y_target, fs2] = audioread([DIR fname '.wav']);

[x_source,fs1] = audioread([DIR_source FILENAME_source '.wav']);
%sound(x,fs);
fs = 16000;
y_target = resample(y_target, fs, fs2);
f_target = pitch(y_target, fs);
y_t_auto = abs(min(autocorr(y_target)));

x_source = resample(x_source,fs,fs1);
x = x_source(:,1);
nx = length(x);

y_s_auto = abs(min(autocorr(x)));

f_source = pitch(x, fs);
avg_t = sum(f_target)/length(f_target);
avg_s = sum(f_source)/length(f_source);
shift = avg_t/avg_s;
% freqShift = log2(shift)*12;
freqShift = -3;
%% ANALYSIS PARAMETERS
...
for m=2:nFrames-1
    ...
    tt = (m-1)*R+1: (m+1)*R;  
    [y_synth,phaseUpdate, freqUpdate, freqTag] = MyAdditivesynth2020(amps(:,m),fRatio*freqs(:,m),R,state,fs, fRatio);
	y(tt)= y(tt) + y_synth;
...
for m=1:nFrames
    tt = (m-1)*R+1: (m)*R;
    if (max(abs(y_sum((m-1)*R+1:(m)*R))) > max(y_sum)*0.1)
        y_t = [y_t; y_sum((m-1)*+R+1:(m)*R)];
    end
end
% y_sum = [y_t; zeros(500,1)];
sound(y_sum,fs);
...
for n=1:nFrames
    if n == 1
        ...
        if (born_line_tag(n) == 1)
            for point = 1:round((n - pre_born_line)*1/100)
    ...
    for i=1:length(cut_line)
        if m == cut_line(i)
            if i == 1
                ...
                if (tmp < (m-1)*R)
                    if (m-1)*R < length(y_sum)
                        if (length(y_new) > R)
                            y_new(end-R:end) = y_new(end-R:end) + y_sum(tmp:tmp + R);
                        end
                        %if (max(abs(y_sum(tmp+R+1:(m-1)*R))) > max(y_sum)*0.1)
...
% y_final = [];
% for t=1:length(y_new)
%     if (abs(y_new(t)) > max(y_new)*0.1)
%         y_final = [y_final; y_new(t)];
%     end
% end
% 
% figure(3)
